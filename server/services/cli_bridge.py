...
class CliBridge:
    ...
    def generate_command(self, opp_id: str) -> str:
        ...
        if has_roster:
            ...
            prompt = (
                f"You are a product strategist framing an investigation opportunity. "
                f"Your job is to TIGHTEN the framing and MOVE ON — not have an open discussion.\n\n"
                f"WORKSPACE: {ws_path}/\n"
                f"OPPORTUNITY: {title}\n"
                f"{tools_line}\n"
                ...
                f"SCOPE CHECK — Before responding on Turn 1, verify these 4 dimensions:\n"
                f"1. Target segment — specific cohort (not 'all users')\n"
                f"2. Geographic scope — named market (not 'multi-market')\n"
                f"3. Investigation type — discovery or optimization\n"
                f"4. Key metric — a specific conversion point (not 'engagement')\n"
                f"If ANY are missing, your 'To sharpen' question MUST target the missing dimension.\n\n"
                ...
                f"TURN 1 — respond with EXACTLY this structure:\n\n"
                f"## How Might We\n"
                f"[Reframe as a specific HMW question — constrained enough to investigate, broad enough for multiple approaches.]\n\n"
                f"## What the data says\n"
                f"[2-3 bullets connecting the idea to real data/strategy from context files — cite specific metrics or strategy references]\n\n"
                f"## To sharpen\n"
                f"[ONE question targeting the most critical gap from the scope check above]\n\n"
                f"TURN 2+ (via --resume):\n"
                ...
                f"title (as a HMW question), type, description, assumptions, success_signals, kill_signals, and context_refs. "
                f"This file write is mandatory on every turn — do not skip it.\n"
                f"- Share what you updated (briefly — 1-2 lines)\n"
                f"- Check the EXIT CHECKLIST below. If all items are met, EXIT IMMEDIATELY.\n"
                f"- If gaps remain, ask ONE question targeting the most critical gap.\n\n"
                f"EXIT CHECKLIST — evaluate EVERY turn:\n"
                f"[ ] HMW title is specific (not vague like 'improve engagement')\n"
                f"[ ] Target segment named\n"
                f"[ ] Geo scope named\n"
                f"[ ] Key metric is a conversion point\n"
                f"[ ] 3+ assumptions stated\n"
                f"[ ] 2+ success signals\n"
                f"[ ] 1+ kill signals\n"
                f"If ALL are met: DO NOT ask another question. Summarize the locked framing and say: "
                f"'Framing is solid. Run `/assemble` to set up the investigation team.'\n\n"
                f"TURN PRESSURE:\n"
                f"- After Turn 2: announce 'Framing is locking' — only ask if a critical gap remains.\n"
                f"- After Turn 4: you MUST exit regardless. Summarize what's solid and flag remaining "
                f"gaps for the investigation team to resolve.\n\n"
                f"Be direct. Each turn must close a gap. Do not extend the conversation if gaps are filled."
            )
